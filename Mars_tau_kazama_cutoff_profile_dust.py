...
ver. 5.2: Look Up Tableの原型を作成 : dust retirieval用の吸収係数の計算を行う
created on Tue Jul 11 14:15:00 2023
...
# HITRAN dataを読み込み
Hitrandata = np.loadtxt('3470-3765_CO2hitrandata.txt')
v_ind = Hitrandata[:, 0]
vij = Hitrandata[:, 0]
Sij = Hitrandata[:, 1]
...
def main():
    ...
    for i in range(len(name_list)):
        ...
        T = np.zeros(len_T)
        T_txt = np.loadtxt(str(name_list[i])+'.txt')
        T = T_txt[:, 2]

        # 圧力
        P = np.zeros(len_T)
        P_txt = np.loadtxt(str(name_list[i])+'.txt')
        P = (P_txt[:, 1])*10  # Pa⇒Barye

        QT = np.zeros(len_T)
        Q_CO2 = pd.read_csv(filepath_or_buffer=str(name_list[i])+".csv",
                            encoding="cp932", sep=",")
        QT = Q_CO2['Q']
        QT = QT.to_numpy()
    ...
    # データセーブ
        ...
        np.savetxt(str(name_list[i])+'_tau.txt', tau_v, fmt='%.10e')
        np.savetxt(str(name_list[i])+'_Kw.txt', new_Kw, fmt='%.10e')
