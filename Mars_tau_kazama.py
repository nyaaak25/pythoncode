...
def Voigt():
    ...
    #-y + 0.195|x|-0.176の計算
    x_y = np.empty((len(vij),len(T),len(v)))
    for i in range(len(v)):
        x_y[:,:,i] = -y + 0.195*abs(x[:,:,i]) - 0.176
    print(x_y)

    # %%
    K1 = np.empty((len(vij),len(T),len(v)))
    K2 = np.empty((len(vij),len(T),len(v)))
    K3 = np.empty((len(vij),len(T),len(v)))
    K4 = np.empty((len(vij),len(T),len(v)))
...
def tau_absorption():
    ...
    for i in range(len(vij)):
        for k in range(len(v)):
            for j in range(len(T)):
                if j==0:
                    tau[i,k]= 0.5 * sigma[i,j,k]*nd[j]*mixCO2*(2e5)
                elif j== len(T)-1:
                    tau[i,k] += 0.5 * sigma[i,j,k] * nd[j] * mixCO2 * (2e5)
                else:
                    tau[i,k]+= 0.5 * sigma[i,j,k]*nd[j] *mixCO2* (4e5)
