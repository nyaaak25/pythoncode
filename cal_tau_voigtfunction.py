...
#ベタ計算
iy1, err1 = (np.zeros((len(vij),len(T), len(ν))), np.zeros((len(vij),len(T), len(ν))))
la = np.zeros((len(vij),len(T), len(ν)))
f = np.zeros((len(vij),len(T), len(ν)))
for i in range(len(vij)):
    for j in range(len(T)):
        for k in range(len(ν)):
            la = lambda t: ((np.e**(-t**2))/((x[i][j,k]-t)**2+y[i][j]**2))
            iy1[i,j,k], err1[i,j,k] = integrate.quad(la, -5, 5)
            f[i,j,k] = (y[i][j]/np.pi)*iy1[i][j,k]/νD[i][j]/np.sqrt(np.pi) #cm^-1
