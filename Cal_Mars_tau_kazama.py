...
# 最後まで使う
ν = np.zeros(101100)
for i in range(101100):
    ν[i] = 4545.0000+(0.001*i)
...
for i in range(len(ν)):
    ...
    K2[:, :, i] = (a3 + b3*x[:, :, i]**2 + c3*x[:, :, i]**4 + d3*x[:, :, i]**6) / \
        (a4 + b4*x[:, :, i]**2 + c4*x[:, :, i]
         ** 4 + d4*x[:, :, i]**6 + x[:, :, i]**8)
    print('第二近似', i)
    ...
    a5 = 272.102 + 973.778*y + 1629.76*y**2 + 1678.33*y**3 + 1174.8*y**4 + \
        581.746*y**5 + 204.501*y**6 + 49.5213*y**7 + 7.55895*y**8 + 0.564224*y**9
    b5 = -60.5644 - 2.34403*y + 220.843*y**2 + 336.364*y**3 + \
        247.198*y**4 + 100.705*y**5 + 22.6778*y**6 + 2.25689*y**7
    c5 = 4.58029 + 18.546*y + 42.5683*y**2 + \
        52.8454*y**3 + 22.6798*y**4 + 3.38534*y**5
    d5 = -0.128922 + 1.66203*y + 7.56186*y**2 + 2.25689*y**3
    e5 = 0.000971457 + 0.564224*y
    a6 = 272.102 + 1280.83*y + 2802.87*y**2 + 3764.97*y**3 + 3447.63*y**4 + \
        2256.98*y**5 + 1074.41*y**6 + 369.199*y**7 + 88.2674*y**8 + 13.3988*y**9 + y**10
    b6 = 211.678 + 902.306*y + 1758.34*y**2 + 2037.31*y**3 + 1549.68 * \
        y**4 + 793.427*y**5 + 266.299*y**6 + 53.5952*y**7 + 5*y**8
    c6 = 78.866 + 308.186*y + 497.302*y**2 + 479.258 * \
        y**3 + 269.292*y**4 + 80.3928*y**5 + 10*y**6
    d6 = 22.0353 + 55.0293*y + 92.7568*y**2 + 53.5952*y**3 + 10*y**4
    e6 = 1.49645 + 13.3988*y + 5*y**2

    K3[:, :, i] = (a5 + b5*x[:, :, i]**2 + c5*x[:, :, i]**4 + d5*x[:, :, i]**6 + e5*x[:, :, i]**8) / \
        (a6+b6*x[:, :, i]**2+c6*x[:, :, i]**4 + d6 *
         x[:, :, i]**6 + e6*x[:, :, i]**8 + x[:, :, i]**10)
    print('第三近似', i)

    del a5, a6, b5, b6, c5, c6, d5, d6, e5, e6

    # Region4　|x|+y < 5.5 and y < 0.195|x|-0.176
    a7 = 1.16028e9*y - 9.86604e8*y**3 + 4.56662e8*y**5 - 1.53575e8*y**7 + 4.08168e7*y**9 - 9.69463e6*y**11 + 1.6841e6 * \
        y**13 - 320772*y**15 + 40649.2*y**17 - 5860.68*y**19 + 571.687 * \
        y**21 - 72.9359*y**23 + 2.35944*y**25 - 0.56419*y**27
    b7 = -5.60505e8*y - 9.85386e8*y**3 + 8.06985e8*y**5 - 2.91876e8*y**7 + 8.64829e7*y**9 - 7.72359e6*y**11 + \
        3.59915e6*y**13 - 234417*y**15 + 45251.3*y**17 - 2269.19 * \
        y**19 - 234.143*y**21 + 23.0312*y**23 - 7.33447*y**25
    c7 = -6.51523e8*y + 2.47157e8*y**3 + 2.94262e8*y**5 - 2.04467e8*y**7 + 2.29302e7*y**9 - 2.3818e7 * \
        y**11 + 576054*y**13 + 98079.1*y**15 - 25338.3*y**17 + \
        1097.77*y**19 + 97.6203*y**21 - 44.0068*y**23
    d7 = -2.63894e8*y + 2.70167e8*y**3 - 9.96224e7*y**5 - 4.15013e7*y**7 + 3.83112e7*y**9 + \
        2.2404e6*y**11 - 303569*y**13 - 66431.2*y**15 + \
        8381.97*y**17 + 228.563*y**19 - 161.358*y**21
    e7 = -6.31771e7*y + 1.40677e8*y**3 + 5.56965e6*y**5 + 2.46201e7*y**7 + 468142 * \
        y**9 - 1.003e6*y**11 - 66212.1*y**13 + \
        23507.6*y**15 + 296.38*y**17 - 403.396*y**19
    f7 = -1.69846e7*y + 4.07382e6*y**3 - 3.32896e7*y**5 - 1.93114e6*y**7 - \
        934717*y**9 + 8820.94*y**11 + 37544.8*y**13 + 125.591*y**15 - 726.113*y**17
    g7 = -1.23165e6*y + 7.52883e6*y**3 - 900010*y**5 - 186682*y**7 + \
        79902.5*y**9 + 37371.9*y**11 - 260.198*y**13 - 968.15*y**15
    h7 = -610622*y + 86407.6*y**3 + 153468*y**5 + 72520.9 * \
        y**7 + 23137.1*y**9 - 571.645*y**11 - 968.15*y**13
    o7 = -23586.5*y + 49883.8*y**3 + 26538.5*y**5 + \
        8073.15*y**7 - 575.164*y**9 - 726.113*y**11
    ...
    t7 = -0.56419*y
    a8 = 1.02827e9 - 1.5599e9*y**2 + 1.17022e9*y**4 - 5.79099e8*y**6 + 2.11107e8*y**8 - 6.11148e7*y**10 + 1.44647e7*y**12 - \
        2.85721e6*y**14 + 483737*y**16 - 70946.1*y**18 + 9504.65 * \
        y**20 - 955.194*y**22 + 126.532*y**24 - 3.68288*y**26 + y**28
    b8 = 1.5599e9 - 2.28855e9*y**2 + 1.66421e9*y**4 - 7.53828e8*y**6 + 2.89676e8*y**8 - 7.01358e7*y**10 + 1.39465e7 * \
        y**12 - 2.84954e6*y**14 + 498334*y**16 - 55600*y**18 + \
        3058.26*y**20 + 533.254*y**22 - 40.5117*y**24 + 14*y**26
    c8 = 1.17022e9 - 1.66421e9*y**2 + 1.06002e9*y**4 - 6.60078e8*y**6 + 6.33496e7*y**8 - 4.60396e7*y**10 + \
        1.4841e7*y**12 - 1.06352e6*y**14 - 217801*y**16 + 48153.3 * \
        y**18 - 1500.17*y**20 - 198.876*y**22 + 91*y**24
    d8 = 5.79099e8 - 7.53828e8*y**2 + 6.60078e8*y**4 + 5.40367e7*y**6 + 1.99846e8*y**8 - 6.87656e6 * \
        y**10 - 6.89002e6*y**12 + 280428*y**14 + 161461 * \
        y**16 - 16493.7*y**18 - 567.164*y**20 + 364*y**22
    e8 = 2.11107e8 - 2.89676e8*y**2 + 6.33496e7*y**4 - 1.99846e8*y**6 - 5.01017e7*y**8 - \
        5.25722e6*y**10 + 1.9547e6*y**12 + 240373*y**14 - \
        55582*y**16 - 1012.79*y**18 + 1001*y**20
    f8 = 6.11148e7 - 7.01358e7*y**2 + 4.60396e7*y**4 - 6.87656e6*y**6 + 5.25722e6 * \
        y**8 + 3.04316e6*y**10 + 123052*y**12 - \
        106663*y**14 - 1093.82*y**16 + 2002*y**18
    g8 = 1.44647e7 - 1.39465e7*y**2 + 1.4841e7*y**4 + 6.89002e6*y**6 + \
        1.9547e6*y**8 - 123052*y**10 - 131337*y**12 - 486.14*y**14 + 3003*y**16
    h8 = 2.85721e6 - 2.84954e6*y**2 + 1.06352e6*y**4 + 280428 * \
        y**6 - 240373*y**8 - 106663*y**10 + 486.14*y**12 + 3432*y**14
    o8 = 483737 - 498334*y**2 - 217801*y**4 - 161461 * \
        y**6 - 55582*y**8 + 1093.82*y**10 + 3003*y**12
    p8 = 70946.1 - 55600*y**2 - 48153.3*y**4 - \
        16493.7*y**6 + 1012.79*y**8 + 2002*y**10
    ...
    K4[:, :, i] = (np.exp(y**2) / np.exp(x[:, :, i]**2)) * np.cos(2*x[:, :, i]*y) - ((a7 + b7*x[:, :, i]**2 + c7*x[:, :, i]**4 + d7*x[:, :, i]**6 + e7*x[:, :, i]**8 + f7*x[:, :, i]**10 + g7*x[:, :, i]**12 + h7*x[:, :, i]**14 + o7*x[:, :, i]**16 + p7*x[:, :, i]**18 + q7*x[:, :, i]**20 + r7*x[:, :, i]**22 + s7*x[:, :, i]
                                                                                      ** 24 + t7*x[:, :, i]**26)/(a8+b8*x[:, :, i]**2+c8*(x[:, :, i])**4 + d8*x[:, :, i]**6 + e8*x[:, :, i]**8 + f8*x[:, :, i]**10 + g8*x[:, :, i]**12 + h8*x[:, :, i]**14 + o8*x[:, :, i]**16 + p8*x[:, :, i]**18 + q8*x[:, :, i]**20 + r8*x[:, :, i]**22 + s8*x[:, :, i]**24 + t8*x[:, :, i]**26 + x[:, :, i]**28))
    print('第4近似', i)
