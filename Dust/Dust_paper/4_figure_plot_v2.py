...
# Histogramのビンの色をlocal timeによって変える
min_lt = 6
max_lt = 18

norm2 = Normalize(vmin=min_lt, vmax=max_lt)
sm2 = ScalarMappable(cmap=cmap, norm=norm2)
sm2.set_array([])

fig, axs = plt.subplots(2,2,dpi=800,figsize=(20, 8),gridspec_kw={'height_ratios': [1, 2]})
# color mapの設定
...
# MY27-29
lt_color = np.zeros(37)
ls_count = np.zeros(37)
bin_edges = np.linspace(0,360,37)

# local timeをLs 10度ごとにわける
for loop in range(0,360,10):
    ind = np.where((ALL_ls >= loop) & (ALL_ls < loop+10))
    ls_count[loop//10] = len(ALL_ls[ind])
    lt_color[loop//10] = np.median(ALL_lt[ind])

color_lt = sm2.to_rgba(lt_color)

# データとビンのエッジを計算
axs[0,0].set_ylabel("Number of orbit", fontsize=20)
axs[0,0].set_title("MY27-29", fontsize=24)

# 各ビンを個別に描画
for i in range(len(bin_edges)-1):
    axs[0,0].bar(bin_edges[i], ls_count[i], width=10, color=color_lt[i], edgecolor='black')

# MY27は0から81まで
# MY28は81から118まで
# MY29は118から最後まで

axs[1,0].set_xlabel("Solar longitude [Deg]", fontsize=20)
axs[1,0].set_ylabel("Latitude [Deg]", fontsize=20)
...
for i in range(count):
    ...
    axs[1,0].scatter(Ls,med_lat, color=color, linewidth=4)
...
# local timeをLs 10度ごとにわける
for loop in range(0,360,10):
    ind = np.where((MY27_all_ls >= loop) & (MY27_all_ls < loop+10))
    ls_count[loop//10] = len(MY27_all_ls[ind])
    lt_color[loop//10] = np.median(MY27_all_lt[ind])

color_lt = sm2.to_rgba(lt_color)

# データとビンのエッジを計算
axs[0,1].set_ylabel("Number of orbit", fontsize=20)
axs[0,1].set_title("MY27", fontsize=24)

# 各ビンを個別に描画
for i in range(len(bin_edges)-1):
    axs[0,1].bar(bin_edges[i], ls_count[i], width=10, color=color_lt[i], edgecolor='black')

axs[1,1].set_xlabel("Solar longitude [Deg]", fontsize=20)
axs[1,1].set_ylabel("Latitude [Deg]", fontsize=20)
axs[1,1].set_xlim(0, 360)
axs[1,1].set_ylim(-65, 65)
...
for i in range(0,82,1):
    ...
    #axs[1].plot([Ls,Ls],[min_lat,max_lat], color=color, linewidth=4)
    axs[1,1].scatter(Ls,med_lat, color=color, linewidth=4)


# カラーバーを作成
cbar = plt.colorbar(sm2, ax=axs[0,1])
...
axs[0,0].set_ylabel("Number of orbit", fontsize=20)
axs[0,0].set_title("MY28", fontsize=24)

# local timeをLs 10度ごとにわける
for loop in range(0,360,10):
    ind = np.where((MY28_all_ls >= loop) & (MY28_all_ls < loop+10))
    ls_count[loop//10] = len(MY28_all_ls[ind])
    lt_color[loop//10] = np.median(MY28_all_lt[ind])
...
# local timeをLs 10度ごとにわける
for loop in range(0,360,10):
    ind = np.where((MY29_all_ls >= loop) & (MY29_all_ls < loop+10))
    ls_count[loop//10] = len(MY29_all_ls[ind])
    lt_color[loop//10] = np.median(MY29_all_lt[ind])

color_lt = sm2.to_rgba(lt_color)

# データとビンのエッジを計算
axs[0,1].set_ylabel("Number of orbit", fontsize=20)
axs[0,1].set_title("MY29", fontsize=24)
