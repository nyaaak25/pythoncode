...
for i in range(1, 5, 1):
    for j in range(1, 5, 1):
        for k in range(1, 5, 1):
            ...
            ARS = np.loadtxt(
                "/Users/nyonn/Desktop/pythoncode/output/SZA"
                + str(i)
                + "_EA"
                + str(j)
                + "_PA"
            ...
            AA[i - 1, j - 1, k - 1] = ARS_y[5]
            Label = SZA_list[i - 1] + ", " + EA_list[j - 1] + ", " + PA_list[k - 1]

            ax.plot(ARS_x, ARS_y, label=Label, zorder=i, lw=1)

ax.set_xlim(2.76, 2.78)
...
# %%
fig = plt.figure(dpi=200)
ax = fig.add_subplot(111, title="dependent of degrees")
ax.grid(c="lightgray", zorder=1)
ax.set_xlabel("PA [deg]", fontsize=10)

for i in range(1, 2, 1):
    for j in range(1, 5, 1):
        for k in range(1, 2, 1):
            # Label = SZA_list[i-1] + ', ' + EA_list[j-1] + ', ' + PA_list[k-1]
            # Label = EA_list[j-1] + ', ' + PA_list[k-1]
            Label = PA_list[k - 1]
            ax.plot(PA, AA[3, :, k - 1], label=Label, zorder=i, lw=1)
