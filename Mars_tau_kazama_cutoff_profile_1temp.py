...
# 波数幅: 1.8 cm-1から2.2m-1までは4545cm-1から5556cm-1, cutoff: 120 cm-1
# plotする範囲の波数
dv = 0.01
cut = 500
...
def main():
    ...
    T = np.zeros(len_T)
    T_txt = np.loadtxt('LookUpTable_HTP/LUTable_T1_285_T2_200_PRS1500.txt')
    T = T_txt[:, 2]

    # 圧力
    P = np.zeros(len_T)
    P_txt = np.loadtxt('LookUpTable_HTP/LUTable_T1_285_T2_200_PRS1500.txt')
    P = (P_txt[:, 1])*10  # Pa⇒Barye

    QT = np.zeros(len_T)
    Q_CO2 = pd.read_csv(filepath_or_buffer="LookUpTable_Q/LUTable_T1_285_T2_200_PRS1500.csv",
                        encoding="cp932", sep=",")
    QT = Q_CO2['Q']
    QT = QT.to_numpy()
...
# データセーブ
    np.savetxt('LookUpTable_Kw/test3_Kw_t1_5_T2_3_SP_15.txt',
               new_Kw, fmt='%.10e')
